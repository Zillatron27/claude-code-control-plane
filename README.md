...
DryDock shipped in 3 days. The changelog tells the story: initial release on Feb 22, then a rapid feature cadence — comparison tables, cherry-pick sourcing, import/export (35 tests), preset blueprints, ship stats dashboard, and permalink sharing (43 more tests) — through to v1.0.0 on Feb 24.
...
The test at the end is simple: The tool works because the formula engine was accurate (spec-driven), the UX was consistent (design system policy), the sharing system was well-defined (spec-driven) and the deployment was clean (skill-driven). The control plane didn't write the code, but it made the code reliable enough to ship publicly with confidence for someone that doesn't know how to write code.

### What this demonstrates

DryDock is not a complex project (even with the complex math). It's a client-side calculator with ~2,500 lines of TypeScript. The point isn't the scale — it's that a non-developer shipped a working, tested, publicly-used tool by treating AI-assisted development as an operations problem rather than a coding problem.
